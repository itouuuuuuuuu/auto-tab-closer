...
- It stores the following data locally using `chrome.storage`:
  - Your settings (threshold, check interval, whitelist, tab-group exclusion) in `chrome.storage.sync`, so Chrome can sync them between your own devices.
  - The pause toggle and a list of recently auto-closed tabs (URL, title, time closed; capped at 100 entries) in `chrome.storage.local`.
