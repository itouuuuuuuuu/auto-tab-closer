...
- Threshold is configurable from 1 to 720 hours.
...
- Toolbar badge shows how many tabs were closed since you last opened the popup.
...
- A `chrome.alarms` alarm fires every 5 minutes. The service worker queries all tabs, builds snapshots, and hands them to a pure function (`src/core/select-tabs.ts`) that decides what to close.
...
- Settings live in `chrome.storage.sync`; the pause toggle, recently-closed list, and badge counter live in `chrome.storage.local`; per-session tracking lives in `chrome.storage.session`.
